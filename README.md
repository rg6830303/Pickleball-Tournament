...
1. Add repository secrets: **Settings → Secrets and variables → Actions →
   ...
   - `SUPABASE_DB_URL` *(required)* — Supabase → Project Settings → Database →
     Connection string → **Session pooler** (the `...pooler.supabase.com:5432...`
     one, with your database password in it)
   - `ADMIN_PASSWORD` *(optional)* — the organiser console password. If you skip
     it, create the organiser login from the console's "Set up my login" panel.
...
3. Organiser login — created only if you first run
   `set mpl.admin_password = 'your-password';` in the same query window
4. All 16 captain logins — `Team1`…`Team16`, each given a freshly generated
   password stored in the staff-only table
...
**No password is stored in this repository** — it is public. Captain passwords
are generated by the setup directly into the staff-only `auction_team_logins`
table and are readable only by a signed-in organiser, in
**Console → Auction → Team Logins** (reveal, copy, CSV, or reset any of them).
Re-running the setup does **not** change passwords you have already handed out.
...
### What it does
- **16 captain logins** — `Team1` … `Team16`, each with its **own** generated
  password, never stored in this repo. Read them in Console → Auction →
  Team Logins. Captains pick their team from a dropdown; the email is built
  for them.
