...
--  SECURITY LOCKDOWN
--
--  sheet_file() was written as an internal helper: sheet_submit()
--  checks you are the captain, sheet_submit_for() checks you are staff,
--  and both then call it. The mistake was assuming that made it
--  private. Postgres grants EXECUTE on a new function to PUBLIC, so
--  sheet_file was callable directly — by anon, with the published key —
--  and it took `p_as_staff` as a PARAMETER. A caller could simply say
--  "I am staff" and skip the deadline, the already-submitted check and
--  the void check, for any team in any tie.
--
--  What that allowed: file another team's line-up, and move their
--  declared trump onto a match they lost. sheet_file rewrites
--  match_results.home_trump / away_trump for the tie, so it worked
--  after the scores were in. -2 points to a call they never made.
--
--  Two fixes, either of which would have been enough:
--    1. the function no longer believes the parameter — it checks
--       auth.uid() itself
--    2. EXECUTE is revoked from PUBLIC and anon on everything that
--       writes, so the internal helper is not reachable at all
--
--  Idempotent: safe to run more than once.
-- ============================================================

-- ------------------------------------------------------------
-- 1) THE FUNCTION STOPS TRUSTING ITS CALLER
-- ------------------------------------------------------------
...
declare
  ...
  v_mine   int;
begin
  -- The caller does not get to declare its own privileges. p_as_staff is
  -- now a request, not a fact: it is granted only if auth.uid() really is
  -- on the staff list.
  ...
  -- Anyone not filing as staff may only ever file for their own team.
...
-- ------------------------------------------------------------
-- 2) NOTHING THAT WRITES IS REACHABLE WITHOUT A LOGIN
--    Postgres hands EXECUTE to PUBLIC on every new function. Every
--    writer below is taken back and re-granted only to authenticated,
--    where each one checks the caller's role for itself.
-- ------------------------------------------------------------
do $$
declare
  fn text;
  writers text[] := array[
    'public.sheet_file(int,int,jsonb,int,boolean)',
    ...
    'public.result_trump(int,int,boolean,boolean)',
    ...
    'public.sheet_board()',
    'public.squad_player_usage(uuid)',
...
begin
  foreach fn in array writers loop
    ...
    exception when undefined_function then
      -- the auction functions differ between installs; skip what is absent
      null;
    end;
  end loop;
end $$;

-- sheet_file is the internal helper. Even a signed-in captain has no
-- business calling it directly — sheet_submit is the door.
revoke all on function public.sheet_file(int,int,jsonb,int,boolean) from authenticated;

-- The login screen fills its team dropdown before anyone has signed in,
-- so this one read stays open on purpose.
